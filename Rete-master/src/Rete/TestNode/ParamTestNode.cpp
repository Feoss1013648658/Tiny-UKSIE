...
bool ParamTestNode::performTest(const Token & token, const WME & wme) const {
	return token.at(conditionNumberOfArg2).get(fieldOfArg2) == wme.get(fieldOfArg1);    //LC 要改概率匹配可能要改这里
}
...
ParamTestNodeVector ParamTestNode::generate(const Condition& c, const ConditionVector& condsHigherUp) {
	...
	for (size_t i = 0; i < condsHigherUp.size(); ++i) {
		...
		if (cond.getType() == Condition::positive) {
			for (auto&& fieldName : { Field::id, Field::attr, Field::value }) {    // LC 需要改？
...
//					dict.insert_or_assign(paramName, std::make_pair(i, fieldName));
					dict.insert(std::make_pair(paramName, std::make_pair(i, fieldName)));
				}
			}
		}
	}
	...
	for (auto&& fieldName : { Field::id, Field::attr, Field::value }) {   // LC 需要改？
		...
		if (Field::isParam(fieldString)) {
			...
			if (it != dict.end()) {
				result.emplace_back(fieldName, it->second.first, it->second.second);  //ParamTestNode(Field::Name fieldOfArg1, size_t conditionNumberOfArg2
